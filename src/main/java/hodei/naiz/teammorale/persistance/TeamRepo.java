...
public interface TeamRepo extends R2dbcRepository<Team,Long> {
    ...
    @Query("WITH deleted AS(DELETE FROM user_teams WHERE team_id=:teamId RETURNING team_id),evaluationsdeleted AS (DELETE FROM evaluation WHERE team_id=:teamId),deletedid AS (SELECT team_id FROM deleted LIMIT 1) SELECT * FROM team WHERE id=(SELECT team_id FROM deletedid);")
